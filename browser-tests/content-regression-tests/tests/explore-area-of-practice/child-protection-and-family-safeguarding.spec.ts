...
test.describe.skip('Child Protection and Family Safeguarding', () => {
    test('User journey via Explore menu @journey', async ({ page }) => {
        ...
        await page.getByRole('link', { name: 'Child protection and safeguarding', exact: true }).click()
        
        await expect(page.locator('h1', { hasText: /^Child protection and safeguarding$/ })).toBeVisible()
        await expect(page).toHaveURL(/.*\/explore-areas-of-practice\/child-protection-safeguarding/)
    ...
    test.describe('Accordian @accordian', () => {
        test('All sections are collapsed when you arrive on page' , async ({ page }) => {
            await page.goto('/explore-areas-of-practice/child-protection-safeguarding')
