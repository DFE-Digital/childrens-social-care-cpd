...
test.describe.skip('Youth justice', () => {
    test('User journey via Explore menu @journey', async ({ page }) => {
        ...
        await page.getByRole('link', { name: 'Youth justice', exact: true }).click()
        
        await expect(page.locator('h1', { hasText: /^Youth justice$/ })).toBeVisible()
        await expect(page).toHaveURL(/.*\/explore-areas-of-practice\/youth-justice/)
    ...
    test.describe('Accordian @accordian', () => {
        test('All sections are collapsed when you arrive on page' , async ({ page }) => {
            await page.goto('/explore-areas-of-practice/youth-justice')
