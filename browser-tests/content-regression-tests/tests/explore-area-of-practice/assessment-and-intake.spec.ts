...
test.describe.skip('Assessment and intake', () => {
    test('User journey via Explore menu @journey', async ({ page }) => {
        ...
        await page.getByRole('link', { name: 'Assessment and intake', exact: true }).click()
        
        await expect(page.locator('h1', { hasText: /^Assessment and intake$/ })).toBeVisible()
        await expect(page).toHaveURL(/.*\/explore-areas-of-practice\/assessment-intake/)
    ...
    test.describe('Accordian @accordian', () => {
        test('All sections are collapsed when you arrive on page' , async ({ page }) => {
            await page.goto('/explore-areas-of-practice/assessment-intake')
